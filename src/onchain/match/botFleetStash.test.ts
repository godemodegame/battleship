...
describe('botFleetStash', () => {
  ...
  it('stashes and peeks both fleets by (deployment, match)', () => {
    stashBotFleets('dep', '7', { player: fleet(0), bot: fleet(3) })
    const got = peekBotFleets('dep', '7')
    expect(got?.player).toEqual(fleet(0))
    expect(got?.bot).toEqual(fleet(3))
  })

  it('isolates entries per deployment and per match id', () => {
    stashBotFleets('dep-a', '1', { player: fleet(0), bot: fleet(1) })
  ...
  it('peeks without consuming — a match spans many shots', () => {
    stashBotFleets('dep', '1', { player: fleet(0), bot: fleet(1) })
  ...
  it('stores defensive copies so a later store wipe cannot mutate the stash', () => {
    ...
    stashBotFleets('dep', '1', { player, bot: fleet(1) })
